...
- **Add NOT NULL constraint** — Enforce non-nullability at the schema level going forward. In PostgreSQL, NOT NULL constraints are enforced — any INSERT or UPDATE that violates the constraint will be rejected. This will fail if any nulls still exist in the column, so always run one of the above remediations first.
...
Add a NOT NULL constraint to prevent future nulls. In PostgreSQL this constraint is enforced — any future INSERT or UPDATE with a null value will be rejected. This will fail if any nulls currently exist — run a fill or delete remediation first.

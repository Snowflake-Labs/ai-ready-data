...
1. **Identify the source system's native identifier.** If the upstream system already emits a correlation or trace ID, propagate it into PostgreSQL rather than generating a new one.
...
2. **Add a trace column to tables that lack one.** Use a standard name from the recognized set (`correlation_id`, `trace_id`, `request_id`, `event_id`, `source_id`, `origin_id`, `record_id`, `lineage_id`):
   ...
   ```sql
   ALTER TABLE {{ schema }}.{{ asset }} ADD COLUMN trace_id VARCHAR;
   ```

...
PostgreSQL does not expose column comments in `information_schema.columns`. Comments are retrieved via `col_description()` from `pg_catalog`. PostgreSQL numeric types include `integer`, `bigint`, `smallint`, `numeric`, `real`, `double precision`, and `decimal`.
...
columns_with_units AS (
    ...
    WHERE
      (column_comment IS NOT NULL AND (
          LOWER(column_comment) LIKE '%usd%'
          OR LOWER(column_comment) LIKE '%dollar%'
          OR LOWER(column_comment) LIKE '%percent%'
          OR LOWER(column_comment) LIKE '%unit%'
          OR LOWER(column_comment) LIKE '%meter%'
          OR LOWER(column_comment) LIKE '%kilogram%'
          OR LOWER(column_comment) LIKE '%second%'
          OR LOWER(column_comment) LIKE '%hour%'
          OR LOWER(column_comment) LIKE '%day%'
      ))
...
SELECT
    ...
    CASE
      ...
      ELSE (SELECT COUNT(*) FROM columns_with_units)::NUMERIC / (SELECT COUNT(*) FROM numeric_columns)::NUMERIC
    END AS value
```

...
Shows every column flagged as a PII candidate (by name pattern heuristic) along with its current protection: `MASKING_POLICY` if a masking policy is applied, `TAG:<tag_name>` if a privacy-related tag is present, or `UNPROTECTED` if neither exists.

Tags checked: `pii`, `sensitive`, `anonymized`, `privacy_category`. Tags indicate awareness but do not enforce protection — only masking policies count toward the check score.

Use this to identify which specific columns need masking policies applied, and to verify that the heuristic PII detection is matching the right columns. Columns flagged incorrectly can be noted as false positives.

`account_usage.policy_references` and `tag_references` both have approximately 2-hour latency.

...
Requires a two-step execution: first run `SHOW DYNAMIC TABLES IN SCHEMA` then query the results via `RESULT_SCAN(LAST_QUERY_ID())` in the same session. Reports each dynamic table's scheduling state, last refresh time, and compliance status.

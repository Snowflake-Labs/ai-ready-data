...
Fraction of tables with change tracking enabled.
...
`change_tracking` status is **not** available in `information_schema.tables` — this check uses `SHOW TABLES` + `RESULT_SCAN`. These two statements must run in the same session; if the session resets between them, `RESULT_SCAN` will fail.

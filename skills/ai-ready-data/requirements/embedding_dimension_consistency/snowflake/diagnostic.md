...
Lists every vector column in the schema with its type, flags whether it matches the most common dimension, and provides a recommendation.

## Context

Identifies the most common `VECTOR` type across all vector columns and marks each column as `CONSISTENT` or `INCONSISTENT`. Columns with a different dimension than the majority may cause failures in vector similarity search or require re-embedding before use.

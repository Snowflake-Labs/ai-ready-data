...
Fraction of base tables tagged with a retention / deletion schedule.

## Context

Counts base tables that carry a retention-related tag in `snowflake.account_usage.tag_references`. A score of 1.0 means every base table has an explicit retention schedule; enforcement (actual deletion or TTL behavior) is a separate concern.
...
Requires `{{ tag_names }}` — comma-separated quoted list, typically `'retention_days','retention_policy','data_retention','ttl'`.

...
Text content length distribution with chunking recommendations.

## Context

Buckets all rows by text length and provides per-bucket recommendations. Use this to understand whether the content needs chunking, concatenation, or is already well-sized for embedding.

Length buckets:
- **NULL** — missing content, exclude or impute
- **EMPTY** — empty strings, exclude from embedding
- **TOO_SHORT (<100)** — may lack semantic content, consider combining with context
- **SHORT (100–500)** / **OPTIMAL (501–2,000)** / **GOOD (2,001–8,000)** — good for embedding, no chunking needed
- **LONG (8,001–32,000)** — consider chunking with overlap for better retrieval
- **TOO_LONG (>32,000)** — must chunk, exceeds most model context windows

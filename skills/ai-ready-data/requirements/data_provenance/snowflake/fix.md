...
Snowflake tables support a `COMMENT` property that can store provenance metadata as free-form text. The check and diagnostic queries look for provenance keywords (`source`, `origin`, `from`, `upstream`, `loaded`, `extracted`) in comments longer than 20 characters.
...
For tables that already have a non-provenance comment, prepend or append provenance details rather than replacing the existing content.

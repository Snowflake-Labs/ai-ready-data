...
Per-object breakdown of documentation status for views, dynamic tables, and materialized views.

## Context

Shows each transformation object with its documentation status: `DOCUMENTED` (comment > 20 chars), `PARTIAL` (comment exists but too short), or `UNDOCUMENTED` (no comment). Use this to identify which specific transformations need documentation added via `COMMENT ON`.

...
Joins `information_schema.columns` against `information_schema.tables` to enumerate every timestamp-family column per base table, then summarises each table's timestamp column count and names. Tables with zero timestamp columns are flagged as `NO_TIMESTAMPS` with a recommendation to add an event-timestamp column.
